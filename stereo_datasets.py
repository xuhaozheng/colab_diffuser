...
#             root_path = '/media/neuralmaster/9d5af100-a900-4e89-bab1-43c8b5025daf/neuromaster/Documents/haozheng/StereoDS/MPI-Sintel-stereo-training-20150305'
#             # new_dataset = SintelStereo(aug_params,root=root_path)*140
#             new_dataset = SintelStereo(aug_params,root=root_path)
...
def fetch_dataset(args):
    """ Create the data loader for the corresponding trainign set """
    image_size = [320, 720]
    aug_params = {'crop_size': image_size, 'min_scale': args.spatial_scale[0], 'max_scale': args.spatial_scale[1], 'do_flip': False, 'yjitter': not args.noyjitter}
    if hasattr(args, "saturation_range") and args.saturation_range is not None:
        aug_params["saturation_range"] = args.saturation_range
    if hasattr(args, "img_gamma") and args.img_gamma is not None:
        aug_params["gamma"] = args.img_gamma
